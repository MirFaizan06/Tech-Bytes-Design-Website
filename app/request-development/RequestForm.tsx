...
export function RequestForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await addDoc(collection(db, 'submissions'), { type: 'request-development', ...form, ip, userAgent: navigator.userAgent, createdAt: serverTimestamp() });
  ...
  return (
    <form id="request-form" onSubmit={handleSubmit} className="glass rounded-2xl p-8 space-y-5">
      ...
      <div>
        <label className="block text-xs font-medium text-muted mb-1.5">Project Details *</label>
        <textarea value={form.message} onChange={update('message')} required rows={5} placeholder="Describe your project, goals, and any specific requirements..." className={inputClass} />
      </div>

      <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-violet-600 hover:bg-violet-500 text-white font-semibold transition-all duration-200 hover:shadow-lg hover:shadow-violet-500/25 disabled:opacity-50 disabled:cursor-not-allowed">
